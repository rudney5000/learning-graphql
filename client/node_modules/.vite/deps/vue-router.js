//#region node_modules/.pnpm/vue-router@3.6.5_vue@2.7.16/node_modules/vue-router/dist/vue-router.esm.js
/*!
* vue-router v3.6.5
* (c) 2022 Evan You
* @license MIT
...
function resolveProps(route, config) {
	switch (typeof config) {
		...
		case "boolean": return config ? route.params : void 0;
...
function normalizeLocation(raw, current, append, router) {
	...
	if (!next.path && next.params && current) {
		...
		} else warn(false, "relative params navigation requires a current route.");
		return next;
	}
		...
		if (this.tag === "a") {
			...
			if (a) {
				...
				for (var event in aData.on) {
					...
					if (event in on) aData.on[event] = Array.isArray(handler$1) ? handler$1 : [handler$1];
				}
				for (var event$1 in on) if (event$1 in aData.on) aData.on[event$1].push(on[event$1]);
...
function createRouteMap(routes, oldPathList, oldPathMap, oldNameMap, parentRoute) {
	...
	if (found.length > 0) warn(false, "Non-nested routes must include a leading slash character. Fix the following routes: \n" + found.map(function(path) {
		return "- " + path;
	}).join("\n"));
	return {
		pathList,
...
function addRouteRecord(pathList, pathMap, nameMap, route, parent, matchAs) {
	...
	if (route.children) {
		...
		route.children.forEach(function(child) {
			addRouteRecord(pathList, pathMap, nameMap, child, record, matchAs ? cleanPath(matchAs + "/" + child.path) : void 0);
		});
	}
	...
	if (route.alias !== void 0) {
		...
		for (var i = 0; i < aliases.length; ++i) {
			...
			addRouteRecord(pathList, pathMap, nameMap, {
				path: alias,
	...
	if (name) {
		...
		else if (!matchAs) warn(false, "Duplicate named routes definition: { name: \"" + name + "\", path: \"" + record.path + "\" }");
	}
}
...
function createMatcher(routes, router) {
	...
	function addRoute(parentOrRoute, route) {
		...
		if (parent && parent.alias.length) createRouteMap(parent.alias.map(function(alias) {
			return {
				path: alias,
				children: [route]
			};
	...
	function match(raw, currentRoute, redirectedFrom) {
		...
		if (name) {
			...
			for (var i = 0; i < pathList.length; i++) {
				var record$1 = pathMap[pathList[i]];
	...
	function alias(record, location, matchAs) {
		var aliasedMatch = match({
			_normalized: true,
			path: fillParams(matchAs, location.params, "aliased route with path \"" + matchAs + "\"")
		});
		if (aliasedMatch) {
...
function matchRoute(regex, path, params) {
	...
	for (var i = 1, len = m.length; i < len; ++i) {
		...
		if (key) params[key.name || "pathMatch"] = typeof m[i] === "string" ? decode(m[i]) : m[i];
	}
	return true;
}
...
History.prototype.confirmTransition = function confirmTransition(route, onComplete, onAbort) {
	...
	runQueue(queue, iterator, function() {
		runQueue(extractEnterGuards(activated).concat(this$1$1.router.resolveHooks), iterator, function() {
			if (this$1$1.pending !== route) return abort(createNavigationCancelledError(current, route));
...
function extractGuard(def, key) {
	if (typeof def !== "function") def = _Vue.extend(def);
...
var HashHistory = /* @__PURE__ */ function(History) {
	...
	HashHistory.prototype.setupListeners = function setupListeners() {
		var this$1$1 = this;
		if (this.listeners.length > 0) return;
		var expectScroll = this.router.options.scrollBehavior;
		var supportsScroll = supportsPushState && expectScroll;
		if (supportsScroll) this.listeners.push(setupScroll());
		var handleRoutingEvent = function() {
			var current = this$1$1.current;
...
var VueRouter = function VueRouter(options) {
	...
	switch (mode) {
		...
		case "abstract":
			...
			break;
		default: assert(false, "invalid mode: " + mode);
	}
};
...
VueRouter.prototype.init = function init(app) {
	...
	if (history instanceof HTML5History || history instanceof HashHistory) {
		var handleInitialScroll = function(routeOrError) {
			...
			if (supportsPushState && expectScroll && "fullPath" in routeOrError) handleScroll(this$1$1, routeOrError, from, false);
		};
...
VueRouter.prototype.resolve = function resolve(to, current, append) {
	...
	return {
		location,
		route,
		href: createHref(base, fullPath, this.mode),
